...
class PerfAttributeBase(metaclass=abc.ABCMeta):
    ...
    def save_data(self,
        ...
        if self.report.empty:
            ...
            weights_cols = [s + '_weight' for s in [BENCHMARK, 'total'] + list(factor_names)]
            self.report = pd.DataFrame(columns=['calcDate', 'total'] + list(factor_names) + num_cols + weights_cols)
        ...
        self.report.loc[apply_date] = [calc_date, total_pnl] \
                                      + list(factor_pnl) \
                                      + [num_total] \
                                      + list(num_factors) \
        ...
        for calc_date, apply_date in zip(all_calculate_dates, all_apply_dates):
            ...
            if not schedule.is_rebalance(apply_date):
                ...
                total_pnl = np.dot(today_holding, returns) \
                            - np.sum(today_holding) / np.sum(evolved_bm) * np.dot(evolved_bm, returns)
                p_matrix = self._evolve(codes, today_holding, evolved_bm, evolved_new_table)
                factor_pnl = returns @ p_matrix \
                             - np.sum(p_matrix, axis=0) / np.sum(evolved_bm) * np.dot(evolved_bm, returns)
            ...
            self.save_data(codes, calc_date, apply_date, today_holding, self.p_table, p_matrix, evolved_bm, total_pnl, factor_pnl, factor_names)
